...
namespace _2DSurviveGameServer._03Svc
{
    ...
    public class NetSvc:SvcRoot<NetSvc>
    {
        ...
        //ConcurrentQueue<MsgPack> msgPackQueue;
        ...
        public override void Update()
        {
            ...
            //如果消息队列里有消息则进行处理（注意：这里为什么在Update里面还要使用while来处理呢？为什么客户端那边同样在Update里面而不使用while来处理呢？因为服务端需要处理很多客户端的消息，并且每个客户端有可能会发大量的消息过来，服务端就必须要能及时处理掉，
            //否则就会出现客户端那边延迟高的现象，而客户端不用while的原因是因为客户端只用处理自己跟服务端的消息，一般来说消息量是很少的，直接使用update来驱动处理就ok）
            while (msgPackQueue.Count > 0)
            {
                //在Update方法中，通过lock (pkgque_lock)确保每次只有一个线程能够访问和处理队列中的消息，从而防止了多线程环境下对消息队列的并发修改问题。
                //这是一种典型的悲观锁策略，简单直接，但频繁加锁解锁可能影响性能。
                lock (pkgque_lock)
                {
                    //从消息队列中取出一个消息包
